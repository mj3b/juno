...
from .enterprise_gpt_connector import EnterpriseGPTManager, create_gpt_config_from_env

class EnterpriseGPTIntegration:
    """
    Enterprise GPT integration supporting multiple providers (OpenAI, T-Mobile, Azure, Custom)
    for enhanced natural language processing in the Jira AI Analytics Agent.
    """
    
    def __init__(self):
        self.gpt_manager = EnterpriseGPTManager()
        ...
        self.preferred_provider = os.getenv('GPT_PREFERRED_PROVIDER', 'openai')
    ...
    def _initialize_providers(self):
        """Initialize GPT providers from environment configuration."""
        try:
            configs = create_gpt_config_from_env()
            
            for name, config in configs.items():
                self.gpt_manager.add_provider(name, config)
                self.usage_stats['provider_usage'][name] = {
                    'requests': 0,
                    'tokens': 0,
                    'cost': 0.0
                }
                self.logger.info(f"Initialized GPT provider: {name}")
            
            # Set preferred provider if available
            if self.preferred_provider in self.gpt_manager.get_available_providers():
                self.gpt_manager.set_default_provider(self.preferred_provider)
                self.logger.info(f"Set default provider to: {self.preferred_provider}")
            elif self.gpt_manager.get_available_providers():
                default = self.gpt_manager.get_available_providers()[0]
                self.gpt_manager.set_default_provider(default)
                self.logger.info(f"Set default provider to: {default}")
            else:
                self.logger.warning("No GPT providers configured. Enhanced features will be disabled.")
                
        except Exception as e:
            self.logger.error(f"Error initializing GPT providers: {e}")
    
    def is_available(self) -> bool:
        """Check if any GPT integration is available."""
        return len(self.gpt_manager.get_available_providers()) > 0
    
    def get_available_providers(self) -> List[str]:
        """Get list of available GPT providers."""
        return self.gpt_manager.get_available_providers()
    
    def enhance_query_understanding(self, query: str, context: Dict = None, provider: str = None) -> Dict[str, Any]:
        """
        ...
        if not self.is_available():
            return {"error": "No GPT providers available"}
        ...
        try:
            ...
            response = self.gpt_manager.generate_completion(
                messages=messages,
                provider=provider,
            ...
            try:
                if response.get('content'):
                    ...
                    result = {"error": "Empty response from GPT provider"}
            ...
            # Track usage
            self._track_usage(response, provider)
    ...
    def generate_intelligent_suggestions(self, current_query: str, jira_context: Dict, provider: str = None) -> List[str]:
        """
    ...
    def explain_analytics_results(self, results: Dict, query: str, provider: str = None) -> str:
        """
        ...
        """
        if not self.is_available():
            return "Analytics results explanation not available (No GPT providers configured)."
        ...
        try:
            ...
            explanation = response.get('content', 'Unable to generate explanation.')
            self._track_usage(response, provider)
    ...
    def manage_conversation_context(self, conversation_history: List[Dict], provider: str = None) -> Dict[str, Any]:
        """
...
Please analyze the context and resolve any references."""
    ...
    def _extract_suggestions_from_text(self, text: str) -> List[str]:
        """Extract suggestions from text response when JSON parsing fails."""
        suggestions = []
        lines = text.split('\n')
    ...
    def _track_usage(self, response: Dict, provider: str = None):
        ...
        used_provider = provider or response.get('provider', 'unknown')
        if used_provider in self.usage_stats['provider_usage']:
            self.usage_stats['provider_usage'][used_provider]['requests'] += 1
            self.usage_stats['provider_usage'][used_provider]['tokens'] += total_tokens
        
        # Estimate cost (approximate pricing)
        estimated_cost = total_tokens * 0.00005  # Rough estimate
        self.usage_stats['total_cost'] += estimated_cost
        
        if used_provider in self.usage_stats['provider_usage']:
            self.usage_stats['provider_usage'][used_provider]['cost'] += estimated_cost
        
        self.logger.info(f"GPT usage ({used_provider}): {total_tokens} tokens, estimated cost: ${estimated_cost:.4f}")
    ...
    def reset_usage_stats(self):
        ...
        self.usage_stats = {
            ...
            'provider_usage': {provider: {'requests': 0, 'tokens': 0, 'cost': 0.0} 
                             for provider in self.gpt_manager.get_available_providers()}
        }
    ...
    def switch_provider(self, provider: str) -> bool:
        ...
        try:
            if provider in self.gpt_manager.get_available_providers():
                self.gpt_manager.set_default_provider(provider)
                self.preferred_provider = provider
                self.logger.info(f"Switched to GPT provider: {provider}")
                return True
            else:
